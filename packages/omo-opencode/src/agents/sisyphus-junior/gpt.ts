...
3. After 3 DIFFERENT approaches fail → STOP and report what you tried clearly`
  ...
  if (zh) return buildGptSisyphusJuniorPromptZh(useTaskSystem, promptAppend)
  if (!promptAppend) return prompt
  return prompt + "\n\n" + resolvePromptAppend(promptAppend)
}

function buildGptSisyphusJuniorPromptZh(
  useTaskSystem: boolean,
  promptAppend?: string
): string {
  const taskDiscipline = buildGptTaskDisciplineSectionZh(useTaskSystem)
  const verificationText = useTaskSystem
    ? "所有任务已标记为已完成"
    : "所有待办已标记为已完成"
...
function buildGptTaskDisciplineSectionZh(useTaskSystem: boolean): string {
  if (useTaskSystem) {
    return `## 任务纪律(不可妥协)
